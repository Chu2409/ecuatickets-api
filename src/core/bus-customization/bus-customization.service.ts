...
import { BusCustomizationArrayResDto, BusCustomizationResDto } from './dto/res/bus-customization.dto';
...
export class BusCustomizationService {
 ...
 async addSeatsToExistingBus(busId: number, configurations: CreateBusCustomizationReqDto[], companyId: number): Promise<BusCustomizationArrayResDto> {
  ...
  for (const singleConfig of configurations) {
    const seatType = await this.repository.findSeatTypeById(singleConfig.seatTypeId);
    if (!seatType) {
      throw new NotFoundException(`Tipo de asiento con el id ${singleConfig.seatTypeId} no encontrado`);
    }

    const seatsOnFloor = await this.repository.findBusSeats(busId);
    const seatsOnSameFloor = seatsOnFloor.filter(seat => seat.floor === singleConfig.floor);
    
    let nextSeatNumber = 1;
    let startingRow = 1;
    
    if (seatsOnSameFloor.length > 0) {
      const maxSeatNumber = Math.max(...seatsOnSameFloor.map(seat => parseInt(seat.seatNumber)));
      ...
      if (validRows.length > 0) {
        ...
        const seatsInLastRow = seatsOnSameFloor.filter(seat => seat.row === maxRowOnFloor).length;
    ...
    if (seatsOnSameFloor.length > 0) {
      const seatsInCurrentRowOnFloor = seatsOnSameFloor.filter(seat => seat.row === currentRow).length;
  ...
  return {
    results,
    totalSeatsAdded: results.reduce((sum, result) => sum + result.seatsAdded, 0),
  ...
  async clearBusSeats(busId: number, companyId: number) {
    ...
    const deletedConfigs = await this.repository.deleteBusSeatConfigurations(busId);
  ...
  async getBusConfiguration(busId: number, companyId: number) {
    ...
    const configurations = await this.repository.findBusSeatConfigurations(busId);
  ...
  async findBusByIdAndCompany(busId: number, companyId: number) {
    ...
    if (!bus) {
      throw new NotFoundException(`Bus con id ${busId} no encontrado o no pertenece a la compañía`);
    }
    return bus;
  }
}