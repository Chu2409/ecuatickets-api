...
export class TemplateSeatsService {
  constructor(
    private readonly templateSeatsRepository: TemplateSeatsRepository,
  ) {}
  ...
  async findById(id: number): Promise<SeatTemplateDto> {
    ...
    if (!template) {
      throw new NotFoundException(
        `Plantilla de asientos con ID ${id} no encontrada`,
      )
    }
  ...
  async create(
    ...
    const template = await this.templateSeatsRepository.create(
  ...
  async update(
    ...
    const template = await this.templateSeatsRepository.update(
      id,
      updateSeatTemplateDto,
    )
    return this.mapToDto(template)
  ...
  private validateUniqueSeats(templateSeats: any[]): void {
    ...
    if (seatNumbers.length !== uniqueSeatNumbers.size) {
      throw new BadRequestException(
        'Hay números de asiento duplicados en la plantilla',
      )
    }

    // Validar que no haya posiciones duplicadas
    const positions = templateSeats.map(
      (seat) => `${seat.row}-${seat.column}-${seat.floor || 1}`,
    )
  ...
  private mapToDto(template: any): SeatTemplateDto {
    return {
      ...
      templateSeats:
        template.templateSeats?.map((seat) => ({
          id: seat.id,
